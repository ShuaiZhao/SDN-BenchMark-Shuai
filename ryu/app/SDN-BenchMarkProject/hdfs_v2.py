...
class HDFSController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        super(HDFSController, self).__init__(*args, **kwargs)
        self.mac_to_port = {}
    ...
    def _packet_in_handler(self, ev):
        ...
        if pkt_tcp:
            ...
            if str(dst_port) == '54310':
                ...
                self.logger.info("\tInstall HDFS flow between %s and %s" % (dst_dpid_name, src_dpid_name))
                ...
                if len(shortest_path) == 1:
                    self.util.install_flows_for_same_switch_v2(
                        shortest_path, 'TCP', src_ip, dst_ip, src_mac, dst_mac, src_port, dst_port, self.dpid_datapathObj, HDFS_IDLE_TIMER, HDFS_HARD_TIMER, msg)
                else:
                    # install flows between hosts and switch
                    self.install_flows_for_hosts_and_attached_switches(hosts, shortest_path, src_ip, dst_ip, src_port, dst_port, src_mac, dst_mac, msg)
        ...
        for h_mac in hosts:
            if count < len(hosts) and count == 0:
                self.util.install_flow_between_host_and_switch_for_TCP_UDP(
                    h_mac, 'TCP', HDFS_PRIORITY, shortest_path[count:count + 2],
                ...
                self.util.install_flow_between_host_and_switch_for_TCP_UDP(
                    h_mac, 'TCP', HDFS_PRIORITY, list([shortest_path[len(shortest_path) - 1], shortest_path[len(shortest_path) - 2]]),
                    count, dst_ip, src_ip, dst_port, src_port, dst_mac, src_mac, self.dpid_datapathObj, SSH_IDLE_TIMER, SSH_HARD_TIMER, msg)
                count += 1
