# hadoopSSH_v1.py
#  Shuai Jack Zhao
# Allow SSh Accecc from Master to Slaves
...
OFP_SWITCHES_LIST_SCRIPT = \
    './scripts/SDN_benchmark_switch_remote_ops/get_switch_ofpbr_datapath_id.sh'
        ...
        if pkt_tcp:
            ...
            if str(dst_port) == '22':
                ...
                if dst_dpid_name == None:
                    self.logger.info("\tcould not find destination switch..............Exiting.........")
                    return

                self.logger.info("\tInstall SSH flow between %s and %s" % (dst_dpid_name, src_dpid_name))
                ...
                # install it in reversed order
                shortest_path = self.util.return_shortest_path(dst_dpid_name, src_dpid_name)
                self.logger.info("\tShortestPath between %s and %s is %s" % (dst_dpid_name, src_dpid_name, shortest_path))
                ...
                if len(shortest_path) == 1:
                    self.util.install_flows_for_same_switch_v2(
                        shortest_path, 'TCP', src_ip, dst_ip, src_mac, dst_mac, src_port, dst_port, self.dpid_datapathObj, SSH_IDLE_TIMER, SSH_HARD_TIMER, msg)
                else:
                ...
                if len(shortest_path) > 2:
                    ...
                    self.util.install_flows_for_rest_of_switches(
                        shortest_path, 'TCP', SSH_PRIORITY, dst_ip, src_ip, dst_mac, src_mac, self.dpid_datapathObj, SSH_IDLE_TIMER, SSH_HARD_TIMER)
